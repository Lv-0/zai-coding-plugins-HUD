...
from .zai_query import query_usage as zai_query_usage

# Suppress log output
logging.basicConfig(level=logging.ERROR)


def try_zai_plugin_query() -> Optional[Dict[str, Any]]:
    """Try to query usage via ZAI/ZHIPU platform plugin API.

    Uses ANTHROPIC_BASE_URL and ANTHROPIC_AUTH_TOKEN environment variables.
    Results are cached for 30 seconds in last_zai_query.json to avoid
    hammering the platform API on every statusLine invocation.
    ...
    try:
        zai_cache_path = (
            Path.home() / ".cache" / "claude-statusbar" / "last_zai_query.json"
        )

        data = None
        try:
            if zai_cache_path.exists():
                raw = json.loads(zai_cache_path.read_text(encoding="utf-8"))
        ...
        if data is None:
            data = zai_query_usage()
            if data is None:
                return None
            data["_cache_time"] = time.time()
            try:
                zai_cache_path.parent.mkdir(parents=True, exist_ok=True)
                zai_cache_path.write_text(json.dumps(data), encoding="utf-8")
            except OSError:
                pass

        processed_limits = data.get("quota_raw", [])
        token_limits = []
        time_limit_item = None
        if isinstance(processed_limits, list):
            for item in processed_limits:
                t = item.get("type", "")
                if t == "TOKENS_LIMIT":
                    token_limits.append(item)
                elif t == "TIME_LIMIT":
                    time_limit_item = item
        elif isinstance(processed_limits, dict):
        ...
        if token_limits:
            shortest = token_limits[0]
            token_pct = shortest.get("percentage")
            nrt = shortest.get("nextResetTime")
            if nrt:
                rate_limit_resets_at = nrt / 1000.0

        if len(token_limits) > 1:
            longest = token_limits[-1]
            weekly_pct = longest.get("percentage")
        ...
        if data.get("mcp_usage_pct") is not None:
            mcp_pct = data.get("mcp_usage_pct")
            mcp_current = data.get("mcp_current_usage")
            mcp_total = data.get("mcp_total")
            mcp_details = data.get("mcp_details")
        elif time_limit_item:
            mcp_pct = time_limit_item.get("percentage")
            mcp_current = time_limit_item.get("currentValue")
            mcp_total = time_limit_item.get("usage")
            mcp_details = time_limit_item.get("usageDetails")

        if token_pct is None and mcp_pct is None:
            return None

        result: Dict[str, Any] = {
            "source": "zai-plugin",
            "platform": data.get("platform", "UNKNOWN"),
            "rate_limit_pct": token_pct,
            "rate_limit_7d_pct": weekly_pct,
            "rate_limit_resets_at": rate_limit_resets_at,
            "mcp_usage_pct": mcp_pct,
            "mcp_current_usage": mcp_current,
            "mcp_total": mcp_total,
        ...
        logging.error(f"ZAI plugin query failed: {e}")
        return None
...
def try_original_analysis() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        possible_paths = [
            ...
            Path.home()
            / ".local/pipx/venvs/claude-monitor/bin/python",  # pipx installation
        ]
...
"""
        ...
        result = subprocess.run(
            [claude_python, "-c", code], capture_output=True, text=True, timeout=10
        )
...
def direct_data_analysis() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        for jsonl_file in sorted(
            data_path.rglob("*.jsonl"), key=lambda f: f.stat().st_mtime
        ):
            try:
                    ...
                    for line in f:
                        ...
                        try:
                            ...
                            if timestamp >= history_cutoff:
                                if (
                                    last_time is None
                                    or (timestamp - last_time).total_seconds()
                                    > 5 * 3600
                                ):
        ...
        if current_session_tokens > 0:
            all_sessions.append(
                {"tokens": current_session_tokens, "cost": current_session_cost}
            )
        ...
        if len(all_sessions) >= 5:
            ...
            token_limit = max(
                session_tokens[min(p90_index, len(session_tokens) - 1)], 19000
            )
            cost_limit = max(
                session_costs[min(p90_index, len(session_costs) - 1)] * 1.2, 18.0
            )
        else:
            # Default limits
...
PLAN_PRESETS = {
    ...
    "pro": {
        ...
        "cost_limit": 18.0,
        "message_limit": 250,
        "weekly_token_limit": 190_000,
        "weekly_msg_limit": 2_500,
    },
    "max5": {
        "token_limit": 88_000,
        "cost_limit": 35.0,
        "message_limit": 1_000,
        "weekly_token_limit": 880_000,
        "weekly_msg_limit": 10_000,
    },
    "max20": {
        "token_limit": 220_000,
        "cost_limit": 140.0,
        "message_limit": 2_000,
        "weekly_token_limit": 2_200_000,
        "weekly_msg_limit": 20_000,
    },
    "custom": {
        "token_limit": 44_000,
        "cost_limit": 50.0,
        "message_limit": 250,
        "weekly_token_limit": 440_000,
        "weekly_msg_limit": 2_500,
    },
    ...
    "zai-lite": {
        ...
        "cost_limit": 50.0,
        "message_limit": 120,
        "weekly_token_limit": 4_000_000,
        "weekly_msg_limit": 1_200,
    },
    "zai-pro": {
        ...
        "weekly_msg_limit": 6_000,
    },
    "zai-max": {
        "token_limit": 8_000_000,
        "cost_limit": 600.0,
        "message_limit": 2_400,
        "weekly_token_limit": 80_000_000,
        "weekly_msg_limit": 24_000,
    },
}
...
PROMO_END = datetime(
    2026, 3, 29, 7, tzinfo=timezone.utc
)  # 3/28 11:59 PM PT = 3/29 07:00 UTC
...
def auto_detect_plan(usage_data: Dict[str, Any]) -> tuple[str, int]:
    ...
    for tier in PLAN_TIERS:
        ...
        if (
            msgs <= preset["message_limit"] * mult
            and tokens <= preset["token_limit"] * mult
        ):
            return tier, mult
...
def apply_plan_override(
    usage_data: Dict[str, Any], plan_name: Optional[str], multiplier: int = 1
) -> Dict[str, Any]:
    ...
    if preset:
        ...
        usage_data["weekly_token_limit"] = (
            preset.get("weekly_token_limit", 0) * multiplier
        )
...
def resolve_plan(
    usage_data: Optional[Dict[str, Any]], cli_plan: Optional[str]
) -> tuple[str, int]:
    ...
    if usage_data:
        ...
        print(
            ...
            "╰─────────────────────────────────────────────────╯".format(
                f"{detected_plan}(x{detected_mult})"
                if detected_mult > 1
                else detected_plan
            ),
            file=sys.stderr,
...
def fetch_usage_data(plan: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    1. ZAI/ZHIPU plugin API (highest priority, if env vars set)
    2. Fresh cache (<30s) -> return immediately
    3. Stale cache -> return stale data, spawn background refresh
    4. No cache -> synchronous fetch (cold start)

    Plan is resolved via: CLI flag > saved config > auto-detect.
    """
    # Try ZAI/ZHIPU plugin API first (highest priority)
    zai_data = try_zai_plugin_query()
    if zai_data is not None:
        effective_plan, mult = resolve_plan(zai_data, plan)
        return apply_plan_override(zai_data, effective_plan, mult)
...
def build_json_output(
    usage_data: Dict[str, Any], reset_time: str, model: str, display_name: str
) -> Dict[str, Any]:
...
def main(
    ...
    try:
        ...
        has_official = (
            stdin_data.get("rate_limit_pct") is not None
            or stdin_data.get("rate_limit_7d_pct") is not None
        )
        ...
        if zai_data is not None:
            token_pct = zai_data.get("rate_limit_pct")
            weekly_pct = zai_data.get("rate_limit_7d_pct")
            mcp_pct = zai_data.get("mcp_usage_pct")
            platform = zai_data.get("platform", "")

            model = display_name if display_name != "Unknown" else model_id
            ctx_used = stdin_data.get("total_input_tokens", 0) + stdin_data.get(
            ...
            mcp_label = ""
            mcp_cur = zai_data.get("mcp_current_usage")
            mcp_tot = zai_data.get("mcp_total")
            if mcp_cur is not None and mcp_tot is not None:
                mcp_label = f" 🔧{mcp_cur}/{format_number(mcp_tot)}({mcp_pct or 0}%)"

            plan_label = (
                f"{saved_plan}@{platform}{mcp_label}"
                if saved_plan
                else f"{platform}{mcp_label}"
            )
                    ...
                    json.dumps(
                        {
                            ...
                            "platform": platform,
                            "rate_limits": {
                                "short_term": {"used_percentage": token_pct},
                                "long_term": {"used_percentage": weekly_pct},
                                "mcp": {
                                    "used_percentage": mcp_pct,
                                    "current_usage": zai_data.get("mcp_current_usage"),
                                    "total": zai_data.get("mcp_total"),
                                },
                            },
                            "model_usage": zai_data.get("model_usage"),
                            "tool_usage": zai_data.get("tool_usage"),
                            "meta": {
                                "model": model_id,
                                "display_name": display_name,
                                "reset_time": reset_time,
            ...
            if json_output:
                ...
                if ctx_size > 0:
                    ...
                    model = re.sub(r"\s*\([^)]*context[^)]*\)", "", model)
                    model = (
                        f"{model}({format_number(ctx_used)}/{format_number(ctx_size)})"
                    )
            ...
            if stdin_data.get("_has_stdin"):
                ...
                if json_output:
                    ...
                    print(
                        f"⚠ Run inside Claude Code statusLine for rate-limit data | {model}"
                    )
