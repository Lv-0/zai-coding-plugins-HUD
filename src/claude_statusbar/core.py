...
def try_zai_plugin_query() -> Optional[Dict[str, Any]]:
    ...
    Returns usage data with rate_limit_pct from the platform's quota API,
    ...
    try:
        ...
        token_pct = data.get("token_usage_pct")
        ...
        result: Dict[str, Any] = {
            ...
            "mcp_current_usage": data.get("mcp_current_usage"),
        ...
        # Cache the raw query result for reuse
        cache_path = Path.home() / ".cache" / "claude-statusbar" / "last_zai_query.json"
        try:
...
def main(json_output: bool = False, plan: Optional[str] = None,
    ...
    try:
        ...
        if zai_data is not None:
            ...
            cfg = load_config()
            saved_plan = cfg.get('plan', '')
            plan_label = f"{saved_plan}@{platform}" if saved_plan else platform

            if json_output:
                print(json.dumps({
                    "success": True, "source": "zai-plugin",
                    "platform": platform,
                    "rate_limits": {
                        "five_hour": {"used_percentage": token_pct},
                        "mcp_monthly": {"used_percentage": mcp_pct,
                ...
                print(format_status_line(
                    msgs_pct=token_pct, tkns_pct=None,
